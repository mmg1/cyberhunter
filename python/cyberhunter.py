...
'''
 NAME: cyberhunter.py | version: 0.1
 CYBERHUNTER Version: 0.1
 AUTHOR: Diego Perez (@darkquasar) - 2018
 DESCRIPTION: Main module that controls the behaviour of the CYBERHUNTER Hunting and IR framework 
...
import importlib
import logging
import outputmods.output as output
import os
import sys
import time
...
def list_targetfiles(pargs):
  ...
  if Path.is_dir(filepath) == False:
    ...
    try:
      ...
      print('Please select a valid filename or directory')
...
def main():
  ...
  logger.info("Starting CYBERHUNTER Hunting Framework")
  
  # CYBERHUNTER ACTION: PARSE
  if pargs.action == "parse":
    logger.info("Starting CYBERHUNTER MultiParser")
  ...
  # CYBERHUNTER ACTION: COLLECT
  if pargs.action == "collect":
    logger.info("Initiating CYBERHUNTER DFIR Collector")
    logger.info("Starting CYBERHUNTER MultiParser")
