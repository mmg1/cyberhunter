'''
 ...
 DESCRIPTION: This module will invoke Eric Zimmerman's "AppCompatCacheParser.exe" to parse the contents of a SYSTEM hive and return a CSV. It requires that AppCompatCacheParser is configured in CYBERHUNTER.conf.
'''
...
class mod:
  ...
  def parse_shim(self, systemhive):
    # Get CYBERHUNTER framework base dir (one level above .\parsermods)
    # CYBERHUNTER_base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__),os.pardir))
  ...
  def execute(self, run_output_pipe=True):
    ...
    # if "./CYBERHUNTERhunter/data/shim_tmp" doesn't exist
